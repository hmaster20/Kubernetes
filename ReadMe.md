...
## Блокировка узлов для удаления подов и их автобалансировке на разрешенные узлы

```shell
# Маркировка узла как не назначаемого предотвращает размещение новых подов на этом Узле планировщиком,
# но не влияет на существующие Поды на Узле, т.е. на работе запущенных подов это не отражается.
# Это полезно в качестве подготовительного шага перед перезагрузкой узла или другим обслуживанием.
# https://kubernetes.io/ru/docs/concepts/architecture/nodes/
